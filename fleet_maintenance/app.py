...
def add_log(vin):
    db = load_db()
    # Your existing logic to extract form data
    log_entry_and_sync(
        vin, 
        ...
        float(request.form.get("cost_parts", 0) or 0), 
        float(request.form.get("cost_labor", 0) or 0), 
        db, 
        track_interval=(request.form.get("action", "log_and_track") == "log_and_track")
    )
    save_db(db)
